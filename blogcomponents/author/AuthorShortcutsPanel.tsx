...
export function AuthorShortcutsPanel() {
  return (
    <div className="panel">
      <div className="panel-header">
        <div className="panel-header-row">
          <h2>Shortcuts</h2>
          <span
            className="config-tooltip panel-tooltip"
            role="tooltip"
            aria-label="Shortcut note"
          >
            <InfoIcon size={16} strokeWidth={2} aria-hidden />
            <span className="config-tooltip-text">
              Note: the default shortcuts used are intended to be used in conjunction with a system level key modifier
              (such as Raycast) that turns a key (such as caps lock) into a Hyper Key, which simulates the combined
              inputs of all 4 common modifiers.
            </span>
          </span>
        </div>
          ...
          <TableBody>
            {shortcuts.map((entry) => (
              <TableRow key={entry.id}>
                ...
                <TableCell>
                  <ShortcutKeys value={entry.value} />
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
